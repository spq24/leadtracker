class AgenciesController < ApplicationController
	...
	def index
		...
		@agency_actions = @agency.companies.actions.where(reviewed: true).filterrific_find(@filterrific).page(params[:page])
		session[:filterrific_actions] = @filterrific.to_hash
		@actiontypes = Actiontype.all
		@nonleadactions = Nonleadaction.all
		@leadactions = Leadaction.all
		@actions_all_time = @agency.companies.actions.where(reviewed: true)
		@actual_leads = @agency.companies.actions.actual_leads
